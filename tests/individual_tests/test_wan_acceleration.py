...
def test_restore_wan_attention_patch_restores_torch_sdpa(monkeypatch):
    original = torch.nn.functional.scaled_dot_product_attention

    def fake_sdpa(*_args, **_kwargs):
        return None

    torch.nn.functional.scaled_dot_product_attention = fake_sdpa
    monkeypatch.setattr(wan_perf, "_ORIGINAL_SDPA", original)
    monkeypatch.setattr(wan_perf, "_SAGE_SDPA_INSTALLED", True)

    try:
        assert wan_perf.restore_wan_attention_patch() is True
        assert torch.nn.functional.scaled_dot_product_attention is original
        assert wan_perf._SAGE_SDPA_INSTALLED is False
    finally:
