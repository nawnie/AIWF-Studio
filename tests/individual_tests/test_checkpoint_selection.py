...
from aiwf.web.app import _preload_default_checkpoint
...
def test_background_preload_skips_when_local_config_cache_missing():
    checkpoint = _checkpoint("realvis", "RealVis")
    backend = MagicMock()
    backend.can_preload_checkpoint_locally.return_value = False
    generation = MagicMock()
    generation.backend = backend
    generation.list_checkpoints.return_value = [checkpoint]
    ctx = MagicMock()
    ctx.generation = generation
    ctx.settings.last_checkpoint_id = "realvis"

    _preload_default_checkpoint(ctx)

    generation.load_checkpoint.assert_not_called()
    backend.can_preload_checkpoint_locally.assert_called_once_with("realvis")
