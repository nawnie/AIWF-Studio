...
def test_pass5_fast_5b_preflight_is_local_only_and_does_not_need_high_low(tmp_path: Path):
    service = _svc(tmp_path)
    missing = service.preflight(WanI2VRequest(runtime_mode=WAN_RUNTIME_FAST_5B))

    assert missing.ok is False
    assert any("Fast 5B mode needs a local Wan Diffusers folder" in error for error in missing.errors)

    base = _write_component_base(service)
    vae = service.flags.resolved_models_dir() / "VAE" / "wan_2.1_vae.safetensors"
    _write_fake_safetensors(vae)
    ready = service.preflight(WanI2VRequest(runtime_mode=WAN_RUNTIME_FAST_5B))

    assert ready.ok is True
    assert ready.model_id == str(base.resolve())
