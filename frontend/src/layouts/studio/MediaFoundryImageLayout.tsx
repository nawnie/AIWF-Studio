...
const _CANVAS_FALLBACK = '/studio-astronaut-canvas.png'
...
export function MediaFoundryImageLayout({
  ...
  const canvasUrl = activeOutput?.url || _CANVAS_FALLBACK
  const canvasThumbUrl = activeOutput?.thumbnailUrl || _CANVAS_FALLBACK
    ...
    <div className="studio-foundry studio-full-surface" aria-label="Media Foundry Image layout">
      ...
      <main className="studio-foundry-stage">
        ...
        <section className="studio-image-canvas">
          ...
          <div className="studio-canvas-frame">
            ...
            <div className="studio-canvas-minimap"><img src={canvasThumbUrl} alt="" /></div>
          </div>
        </section>

        <section className="studio-foundry-bottom-dock">
          <header>
          ...
          {dockMode === 'scenes' ? (
            <div className="studio-scene-board">
              <div className="studio-session-stats">
              ...
              <div className="studio-scene-strip">
                ...
                <button type="button" className="studio-new-variant" onClick={onGenerate} disabled={isGenerating}>
                  <Plus size={22} />
                  New Variant
                </button>
                <button type="button" className="studio-new-variant" onClick={() => onSendToWorkflow?.('Media Foundry variant')}>
                  <Sparkles size={20} />
                  Send to workflow
                </button>
              </div>
            ...
            <div className="studio-track-board">
      ...
      <aside className="studio-foundry-inspector">
        ...
        {inspectorMode === 'metadata' ? (
          <div className="studio-metadata-panel">
            ...
            <InspectorSection title="Layers">
              ...
              {LAYERS.map((layer, index) => (
                <button
                  key={layer}
                  type="button"
                  className={activeLayer === layer ? 'studio-layer-row active' : 'studio-layer-row'}
                  onClick={() => setActiveLayer(layer)}
                >
                  {index < 5 ? <Eye size={14} /> : <EyeOff size={14} />}
                  <span className="studio-layer-thumb">{assetRows[index % Math.max(1, assetRows.length)] ? <img src={assetRows[index % Math.max(1, assetRows.length)].thumbnailUrl} alt="" /> : null}</span>
                  <span>{layer}</span>
...
function Stat({ label, value }: { label: string; value: string }) {
  return <div className="studio-stat"><span>{label}</span><strong>{value}</strong></div>
}

function TrackRow({ label, title, color, blocks }: { label: string; title: string; color: string; blocks: string[] }) {
  return (
    <div className={`studio-track-row track-${color}`}>
