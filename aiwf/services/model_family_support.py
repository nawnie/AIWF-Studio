...
def static_model_family_support() -> list[dict[str, Any]]:
    ...
    return [
        ...
        _family(
            ...
            category="image",
            status="blocked-until-runtime",
            summary="Anima 2B split assets are cataloged for lower VRAM anime/non-photorealistic generation, but AIWF does not yet include a native Anima loader.",
            storage=["Comfy split .safetensors transformer"],
            precisions=[
                _precision("BF16/FP16", "cataloged", "Comfy split asset"),
            ],
            routes=[_route("anima", "unsupported-no-route", "txt2img", "pending native split-file loader")],
            sidecars=["Qwen 3 0.6B text encoder", "Qwen Image VAE"],
            lora="Anima Turbo LoRA and adapters need a dedicated runtime path before UI exposure.",
            blockers=["Upstream release is ComfyUI-native split files; AIWF needs loader implementation and smoke receipts."],
            modules=["aiwf.services.pipeline_preflight", "aiwf.services.model_download_catalog"],
        ),
        _family(
            id="qwen_image",
