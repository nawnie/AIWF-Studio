...
def _parse_hf_reference(url_or_repo: str, filename: str = "") -> ParsedRemote:
...
class ModelDownloadService:
    ...
    def parse_reference(
        ...
        *,
        ...
        if source == "huggingface":
            return _parse_hf_reference(url_or_repo, filename)
        if source == "civitai":
    ...
    def _catalog_to_remote(self, entry: CatalogEntry) -> ParsedRemote:
        if entry.source == "huggingface":
            return _parse_hf_reference(entry.repo_id, entry.filename)
    ...
    def download_custom(
        ...
        *,
        ...
        remote = self.parse_reference(source=source, url_or_repo=url_or_repo, filename=filename)
    ...
    def folder_paths_help(self) -> str:
        lines = ["**Category → folder**"]
        for key, label in CATEGORY_LABELS.items():
            path = self.destination_dir(key)
            lines.append(f"- **{label}** → `{path}`")
        lines.append(
            "\nSet `HUGGINGFACE_TOKEN` / `HF_TOKEN` for gated Hugging Face repos. "
            "Set `CIVITAI_API_TOKEN` for CivitAI models that require login."
        )
        return "  \n".join(lines)