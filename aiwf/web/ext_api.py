...
class ExtensionTabsPayload(BaseModel):
...
class AgentPermissionsPayload(BaseModel):
    observe: bool = True
    suggest: bool = True
    draft: bool = True
...
class PluginManifestPayload(BaseModel):
    manifest: dict[str, Any] = Field(default_factory=dict)


class ExportPlanPayload(BaseModel):
...
class QaAnalyzePayload(BaseModel):
    projectId: str = "default"
    prompt: str = ""
    assetPath: str = ""
    workflow: dict[str, Any] = Field(default_factory=dict)
...
def _ext_runtime_dir(ctx: Any) -> Path:
    root = _data_root(ctx) / "extensions" / "media-center-v4"
...
def _read_json(path: Path, default: Any) -> Any:
    try:
        ...
        logger.debug("Could not read media center JSON: %s", path, exc_info=True)
    return default
...
def _tabs_path(ctx: Any) -> Path:
    return _ext_runtime_dir(ctx) / "tabs.json"


def _workspace_path(ctx: Any, workspace_id: str) -> Path:
    return _ext_runtime_dir(ctx) / "workspaces" / f"{_safe_id(workspace_id, 'workspace')}.json"


def _projects_root(ctx: Any) -> Path:
    path = _ext_runtime_dir(ctx) / "projects"
...
def _workflow_path(ctx: Any, workflow_id: str) -> Path:
    return _ext_runtime_dir(ctx) / "workflows" / f"{_safe_id(workflow_id, 'workflow')}.json"


def _queue_path(ctx: Any) -> Path:
    return _ext_runtime_dir(ctx) / "render_queue.json"


def _plugins_path(ctx: Any) -> Path:
    return _ext_runtime_dir(ctx) / "plugin_registry.json"


def _permissions_path(ctx: Any) -> Path:
    return _ext_runtime_dir(ctx) / "agent_permissions.json"
...
def _load_tabs(ctx: Any) -> list[dict[str, Any]]:
    ...
    for item in tabs:
        if not isinstance(item, dict):
            continue
        try:
            normalized.append(ExtensionTab.model_validate(item).model_dump(mode="json"))
        except Exception:
            logger.debug("Skipping invalid extension tab payload", exc_info=True)
    return normalized


def _save_tabs(ctx: Any, tabs: list[ExtensionTab]) -> dict[str, Any]:
    payload = {
        "schema": "aiwf.extensions.tabs.v4",
        "updatedAt": _now(),
...
def _worker_receipts_path(ctx: Any) -> Path:
    return _ext_runtime_dir(ctx) / "worker_receipts.json"


def _worker_logs_dir(ctx: Any) -> Path:
    return _ext_runtime_dir(ctx) / "job_logs"


def _get_worker(ctx: Any) -> ext_worker.QueueWorker:
    return ext_worker.get_worker(ctx, _queue_path(ctx), _worker_receipts_path(ctx), _worker_logs_dir(ctx))
...
def build_extension_router(ctx: Any) -> APIRouter:
    router = APIRouter(prefix="/api/pro")

    @router.get("/extensions/manifest")
    def extension_manifest() -> dict[str, Any]:
        return {
            "schema": "aiwf.extensions.manifest.v4",
            "name": "AIWF  Media Center Extension Host",
            "version": "4.0.0",
            "capabilities": [
    ...
    def save_extension_tabs(payload: ExtensionTabsPayload) -> dict[str, Any]:
        return _save_tabs(ctx, payload.tabs)

    @router.post("/extensions/register-tab")
    def register_extension_tab(tab: ExtensionTab) -> dict[str, Any]:
        current = [ExtensionTab.model_validate(item) for item in _load_tabs(ctx)]
    ...
    def extension_workspace(workspace_id: str) -> dict[str, Any]:
        ...
        if isinstance(payload, dict):
            return payload
        return {
            "schema": "aiwf.extensions.workspace.v4",
            "id": workspace_id,
    ...
    def validate_workflow(payload: WorkflowValidatePayload) -> dict[str, Any]:
    ...
    def add_queue_job(payload: QueueJobPayload) -> dict[str, Any]:
    ...
    def qa_analyze(payload: QaAnalyzePayload) -> dict[str, Any]:
    ...
    def export_plan(payload: ExportPlanPayload) -> dict[str, Any]:
        ...
        path = _ext_runtime_dir(ctx) / "exports" / f"{plan['id']}.json"
    ...
    def install_plugin_manifest(payload: PluginManifestPayload) -> dict[str, Any]:
        ...
        if ui.get("leftRail"):
            tab = ExtensionTab(id=plugin_id, label=plugin["name"], workspaceType="empty", description="Plugin workspace")
    ...
    def agent_permissions() -> dict[str, Any]:
        ...
        if isinstance(payload, dict):
            return payload
        default = AgentPermissionsPayload(allowedTools=["project-reader", "workflow-json", "prompt-refiner", "log-viewer"]).model_dump(mode="json")
    ...
    def save_agent_permissions(payload: AgentPermissionsPayload) -> dict[str, Any]:
    ...
    def _build_chat_messages(payload: AgentChatPayload) -> list[dict[str, str]]:
        permissions = _read_json(_permissions_path(ctx), {})
        system_hint = (
            "You are AIWF Studio Agent inside a local AI media creation center. "
            "You can inspect project state, draft workflows, draft patches, explain logs, and propose plugins. "
    ...
    def agent_chat_stream(payload: AgentChatPayload) -> StreamingResponse:
        model = payload.model.strip()
        if not model:
            raise HTTPException(status_code=422, detail="Select an Ollama model first.")
        request_payload = {
