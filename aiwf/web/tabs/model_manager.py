...
def register_model_manager(registry: WebRegistry) -> None:
    ...
    def build(ctx: AppContext, tab: gr.Tab | None = None) -> None:
                ...
                with gr.Tab("Download", elem_classes=["aiwf-model-tab"]):
                    ...
                    with gr.Column(elem_classes=["aiwf-panel"]):
                        ...
                        custom_filename = gr.Textbox(
                            ...
                            info="Required for HF repos unless the URL already points at a file.",
                        )
        ...
        def _lora_form_values(catalog_svc, lora):
            ...
            return (
                catalog_svc.alias_for_lora(lora.id) or "",
                catalog_svc.lora_strength(lora.id),
                catalog_svc.lora_keywords(lora.id, metadata),
            )
        ...
        def save_lora_settings(lora_id: str | None, alias: str, strength: float, keywords: str):
            ...
            ctx.save_settings()
            lora = catalog.find_lora(lora_id)
            token = catalog.keyword_token(alias.strip().lower() or lora_id)
            return (
                catalog.lora_details(lora),
                gr.update(choices=catalog.lora_choices(), value=lora_id),
                f"**Saved** — use `{token}` in Studio prompts.",
            )

        save_lora.click(
            save_lora_settings,
            inputs=[lora_select, lora_alias, lora_strength, lora_keywords],
            outputs=[lora_details, lora_select, lora_status],
        )

        def copy_shortcut_text(lora_id: str | None, alias: str):
            if not lora_id:
                raise gr.Error("Select a LoRA first.")
            token = catalog.keyword_token(alias.strip().lower() or lora_id)
            return f"**Shortcut** `{token}` — paste into the Studio prompt."
        ...
        def load_tab():
            checkpoints = catalog.refresh_checkpoints()
            loras = catalog.refresh_loras()
            ckpt_choices = catalog.checkpoint_choices()
            lora_choices = catalog.lora_choices()
            ckpt_default = ckpt_choices[0][1] if ckpt_choices else None
            lora_default = lora_choices[0][1] if lora_choices else None
            lora = catalog.find_lora(lora_default)
            return (
                gr.update(choices=ckpt_choices, value=ckpt_default),
                catalog.checkpoint_details(catalog.find_checkpoint(ckpt_default)),
                gr.update(choices=lora_choices, value=lora_default),
                catalog.lora_details(lora),
                *_lora_form_values(catalog, lora),
                catalog.models_folder_help(),
                f"**{len(checkpoints)}** checkpoints · **{len(loras)}** LoRAs",
            )
