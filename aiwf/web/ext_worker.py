"""Background render-queue worker for the media center (v5 worker pass).
...
def register_executor(kind: str, fn: Executor) -> None:
    ...
    logger.info("ext_worker: registered executor for kind=%s", kind)
...
class QueueWorker:
    ...
    def _load_queue(self) -> dict[str, Any]:
        try:
            ...
            logger.debug("ext_worker: could not read queue file", exc_info=True)
    ...
    def _append_log(self, job_id: str, line: str) -> None:
        try:
            ...
            logger.debug("ext_worker: could not append job log", exc_info=True)
        ...
        try:
            if executor is None:
                raise RuntimeError(
                    ...
                    "Register one via aiwf.web.ext_worker.register_executor()."
                )
    ...
    def start(self) -> bool:
        ...
        self._thread = threading.Thread(target=self._loop, name="aiwf-queue-worker", daemon=True)
        self._thread.start()
        return True
    ...
    def _loop(self) -> None:
        logger.info("ext_worker: loop started")
        while not self._stop.is_set():
            try:
                if self.run_next() is None:
                    self._stop.wait(1.5)
            except Exception:  # noqa: BLE001
                logger.exception("ext_worker: loop error")
                self._stop.wait(3.0)
        logger.info("ext_worker: loop stopped")
