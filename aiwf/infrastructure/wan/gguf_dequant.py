"""On-the-fly GGUF dequantization (ComfyUI-GGUF dequant.py, Apache-2.0, adapted)."""
...
class GGMLTensor(torch.Tensor):
    """Tensor subclass holding mmap'd GGUF quantized payload."""

    @staticmethod
    def __new__(cls, data, *, tensor_type, tensor_shape, **kwargs):
        return torch.Tensor._make_subclass(cls, data, require_grad=False)

    def __init__(self, *args, tensor_type, tensor_shape, **kwargs) -> None:
        self.tensor_type = tensor_type
        self.tensor_shape = tensor_shape

    def __repr__(self) -> str:
        qname = getattr(self.tensor_type, "name", repr(self.tensor_type))
        return f"GGMLTensor(shape={tuple(self.tensor_shape)}, qtype={qname})"
    ...
    def shape(self):
        ...
        if ts is not None:
            return ts
        return tuple(torch.Tensor(self).size())
...
def dequantize_tensor(tensor, dtype=None, dequant_dtype=None):
    ...
    if qtype in dequantize_functions:
        ...
        payload = tensor.data if hasattr(tensor, "data") else tensor
        payload = torch.Tensor(payload)
        return dequantize(payload, qtype, oshape, dtype=dequant_dtype).to(dtype)
    logger.warning(
        "Falling back to numpy dequant for qtype: %s",
        getattr(qtype, "name", repr(qtype)),
    )
    import numpy as np
