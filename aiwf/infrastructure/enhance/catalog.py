...
def _search_roots(flags: RuntimeFlags) -> dict[str, list[Path]]:
    ...
    roots = {"upscaler": [], "restorer": []}
    seen: dict[str, set[str]] = {"upscaler": set(), "restorer": set()}
    model_roots = [flags.resolved_models_dir(), *flags.resolved_extra_model_dirs()]

    def add(kind: str, path: Path) -> None:
        resolved = path.resolve()
        key = os.path.normcase(str(resolved))
        if resolved.exists() and key not in seen[kind]:
            seen[kind].add(key)
            roots[kind].append(resolved)

    for models_dir in model_roots:
        if models_dir.name.lower() in {"realesrgan", "esrgan", "upscale_models"}:
            add("upscaler", models_dir)
        if models_dir.name.lower() in {"gfpgan", "codeformer", "facerestore_models"}:
            add("restorer", models_dir)
        for candidate in (
