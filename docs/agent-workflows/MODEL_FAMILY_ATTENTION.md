...
- Flux, Flux Kontext, Flux.2 Klein, and Z-Image: preserve native transformer attention processors. Prefer Diffusers `set_attention_backend` when the transformer exposes it; otherwise stay on native torch SDPA. Do not use the global SageAttention patch for Flux.2 Klein.
...
- The July 2026 CUDA 13 benchmark moved the main launch default to SDPA. SageAttention and xFormers remain opt-in until each model family has a saved-image smoke receipt.
...
- Runtime experiment diagram: `docs/experiments/runtime-attention-cuda13.md`
