# AIWF Studio — v2 Roadmap Implementation Plan
...
| Area | Files | Status |
|------|-------|--------|
| GPU tenant lock | `services/gpu_tenant_lock.py` | ✅ done |
| Engine supervisor | `services/engine_supervisor.py` | ✅ done |
| Engine events domain | `core/domain/engine_events.py` | ✅ done |
| Job status domain | `core/domain/job_status.py` | ✅ done |
| Training domain models | `core/domain/training.py` | ✅ done |
| ED2 subprocess client | `services/ed2_client.py` | ✅ done |
| Kohya subprocess client | `services/kohya_client.py` | ✅ done |
| Engine worker stubs | `engines/{ed2,kohya,generation}/worker.py` | ✅ done |
| Wan I2V pipeline | `infrastructure/wan/pipeline.py` | ✅ done |
| Wan service + GPU lock | `services/wan.py` | ✅ done |
| Video tab UI | `web/tabs/wan_i2v.py` | ✅ done |
| Model download catalog | `services/model_download_catalog.py` | ✅ done |
| GGUF/safetensors header reader | `infrastructure/model_header.py` | ✅ done |
| Model swap optimization | `infrastructure/wan/pipeline.py` (swap_models) | ✅ done |
| Engine isolation arch doc | `docs/architecture/engine_isolation_architecture.md` | ✅ done |
| Video domain models | `core/domain/video.py`, `core/domain/wan.py` | ✅ done |
| RIFE service + tab | `services/rife.py`, `web/tabs/rife.py` | ✅ done |
...
2. UI callbacks call services only — no torch/diffusers in tab files.
...
6. `subprocess.Popen([...], shell=False)` only — never raw shell strings.
...
## Sprint A — Engine Domain + Process Supervisor + Ollama Chat
...
Wire into `engine_supervisor.py` — replace its informal string state with `EngineTenant`.

**Test:** `tests/test_engine_domain.py` — switch logic, idle/chat/video transitions.

### A2 — `aiwf/core/domain/worker.py`
...
**Test:** pure dataclass — import and construct.

### A3 — `aiwf/services/process_supervisor.py`

Single place to launch, stream, and kill subprocess workers. Used by ED2Runner, KohyaRunner, and eventually video workers.

Key points:
- `start(worker_id, command) -> Iterator[str]` — streams log lines
- `stop(worker_id) -> str` — psutil recursive kill
- `CREATE_NEW_PROCESS_GROUP` on Windows, `os.setsid` on POSIX
- Never `shell=True`
- Tracks active processes in `dict[str, Popen]`

**Test:** `tests/test_process_supervisor.py` — echo subprocess, stop, double-start guard.

### A4 — `aiwf/services/ollama_client.py`

Thin HTTP wrapper around Ollama's REST API. No mandatory install — gracefully degrades.
...
Dependencies: `httpx` (already in requirements via fastapi chain — verify).  
Fallback: if `httpx` missing, raise `ImportError` with install hint.

**Test:** `tests/test_ollama_client.py` — mock httpx, healthcheck True/False, stream tokens.

### A5 — `aiwf/web/tabs/chat_workspace.py`
...
- Unload button (explicit — does not auto-unload)
- Setup note if Ollama not detected

Wiring rules:
- Tab select → check Ollama health, refresh model list
- Send → acquire GPU lock (CHAT tenant), stream tokens to chatbot
- Unload → release lock, call `client.unload(model)`
- If Ollama not running → show friendly install guidance, no crash

**Does NOT:** auto-download Ollama, auto-load on startup, share VRAM with video/image.

### A6 — Wire chat unload into EngineSupervisor
...
**Test:** `tests/test_dataset_validator.py` — tmp_path fixtures.

### B2 — `aiwf/services/training/kohya_config.py`

Builds Kohya TOML config from `KohyaLoRARequest` domain model. No subprocess logic here — pure config generation.

**Test:** `tests/test_training_config_builders.py` — field mapping, path escaping, repeats syntax.

### B3 — `aiwf/services/training/kohya_runner.py`
...
- Engine lock check — block if GPU held by video/chat

---

## Sprint C — Model Info Lookup (Task #36)

**Goal:** Fetch metadata from HF API, CivitAI API, and Ollama by model name/URL. Display in Models tab info panel.

### C1 — `aiwf/services/metadata.py` (expand existing)

`metadata.py` already exists — extend it:
- `fetch_hf_model_info(repo_id) -> ModelInfo | None` — HF `/api/models/{repo_id}`
- `fetch_civitai_model_info(model_id_or_url) -> ModelInfo | None` — CivitAI `/api/v1/models/{id}`
- `fetch_ollama_model_info(model_name) -> ModelInfo | None` — Ollama `/api/show`
- `ModelInfo` dataclass: name, description, tags, license, downloads, size_mb, url

### C2 — Wire into Models tab

In `model_manager.py`: add "Model Info" panel — enter model name/URL, click Lookup, display info card.

---

## Sprint D — Docs + AGENTS.md Addendum

Write the docs the roadmap calls for (these are not optional — they keep agents aligned):

- `docs/WORKER_PROTOCOL.md` — event types, command/result schemas, streaming contract
- `docs/OLLAMA_CHAT_TENANT.md` — chat architecture, unload behavior, GPU lock interaction
- `docs/TRAINING_ENGINE_ROADMAP.md` — Kohya and ED2 subprocess design
- `docs/ACCELERATION_EXPERIMENTS.md` — what's experimental, what's verified, benchmark protocol
- Append AGENTS.md addendum from roadmap §6

---

## Sprint E — Performance Experiments (deferred, flagged)
...
```
Sprint A  →  Sprint B  →  Sprint C  →  Sprint D  →  Sprint E (optional)
   |              |             |
