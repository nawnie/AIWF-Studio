# Agentic Prompt / Tool Workspace — Phase 7 Roadmap

**Status:** In Progress  
**Phase:** 7 (depends on stable engine boundaries from Phases 0–6)

---

## Goal

Add a structured local-tool layer that lets an embedded assistant (Claude or compatible model) inspect the user's local checkpoint/LoRA library, build prompts, recommend generation settings, and submit requests — all without unsafe shell access, file mutations, or unconfirmed GPU-heavy operations.
...
### Tier 1 — Pure Read (safe to call at any time)
...
### Tier 2 — Compute / Compose (CPU-only, no GPU)
...
### Tier 3 — Side-Effect (require confirmation or GPU-lock check)
...
### Step 1 — Service layer (`aiwf/services/prompt_tools.py`)
...
### Step 2 — Safetensors metadata reader
...
`build_prompt_draft(subject, style_name, lora_names, negative)` — concatenates with correct weight syntax and LoRA activation tags. Pure string operations.

### Step 4 — Settings recommender

Rule-based lookup table keyed on `(base_model_class, goal)`. No ML inference. Returns a `RecommendedSettings` dataclass.

### Step 5 — Tests

`tests/test_prompt_tools.py` — covers all Tier 1 and Tier 2 tools. Uses `tmp_path` fixtures to mock checkpoint directories; no GPU required.

### Step 6 — UI integration (future sprint)

A collapsible "Prompt Tools" accordion in the Studio tab or a dedicated lightweight tab. Out of scope for this sprint — service layer ships first, UI wired after tests pass.
...
| Dependency | Notes |
|------------|-------|
| `pathlib` | stdlib — always present |
| `json` | stdlib — always present |
| `struct` | stdlib — for safetensors header parse |
| `safetensors` (optional) | Only for full metadata; fallback to manual header parse if absent |
| `aiwf.core.context.AppContext` | Required — provides all root paths |
