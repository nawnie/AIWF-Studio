...
To clone Shawn's ED2 fork instead of upstream, set `AIWF_ED2_REPO_URL` before
running the Training tab installer. The installer always targets
...
1. Add an entry to `engines.json` with `repo_dir` and `venv_dir`.
