...
**Status:** Normative â€” implementation must conform to this document.
...
1. **Prompt injection** â€” malicious content in filenames, model metadata, or prompt library entries causes the assistant to take destructive actions.
2. **Privilege escalation** â€” a tool intended for reading metadata is abused to execute arbitrary code.
3. **Silent side effects** â€” GPU-heavy or irreversible operations triggered without user awareness.
4. **Path traversal** â€” a crafted filename escapes the expected root directory.
...
### R1 â€” No raw shell command tool
...
### R4 â€” No arbitrary Python execution
...
### R5 â€” No unconfirmed training launch
...
### R6 â€” GPU lock check before generation

Any tool that would submit a generation request (image or video) must check the GPU tenant lock. If the lock is held by a non-idle tenant, the tool returns an error â€” it does not queue silently or wait.
...
### P1 â€” Resolve through AppContext only
...
### P2 â€” Resolve and validate before use
...
### P3 â€” Extension allowlist for read tools
...
### M1 â€” Header-only safetensors reads
...
### M3 â€” Sanitize returned strings
...
| Submit generation request (GPU busy) | Blocked â€” no confirmation path |
| Start training run | Yes â€” always |
| Delete any file | Blocked â€” no tool exists |
| Write outside output_dir | Blocked â€” no tool exists |
